describe('In IAM module', function() {
    ...
    describe('while testing errors in add identity', function() {
        ...
        describe('adding a new social identity to an existing user', function() {
            ...
            it('an error [409] is returned ' +
            'while trying to create an identity with the same oauth server', function(done) {
                corbelDriver.iam.user(userId).addIdentity({
                    'oauthService': 'facebook',
                    'oauthId': random
                })
            ...
            it('should fail [401] returning an unauthorized error', function(done) {
                var corbelDefaultDriver = corbelTest.drivers['DEFAULT_USER'].clone();
                corbelDefaultDriver.iam.user(userId).addIdentity({
                    'oauthService': 'facebook',
                    'oauthId': random
                })
                .should.be.eventually.rejected
