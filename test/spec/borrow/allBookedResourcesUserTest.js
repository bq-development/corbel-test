describe('In BORROW module, when a user reservers multiple resources', function() {
    ...
    before(function(done) {
        ...
        then(function(createdLoanableResourcesFromPromise) {
            ...
            return corbelTest.common.utils.createNewTestUser(corbelDriverAdmin, userName).
            should.eventually.fulfilled;
        }).
        then(function(id) {
            userTestIdentifier = id;

            return corbelTest.common.utils.loginTestUser(userName).
            should.be.eventually.fulfilled;
        }).
        then(function(loginResponse) {
            corbelDriverUser = loginResponse.driver;

            return bookForResourcesLoanables(corbelDriverUser, userTestIdentifier).
            should.be.eventually.fulfilled;
        }).
        should.notify(done);
    });
    ...
    it('user can get them and is returned status OK (200)', function(done) {
        corbelDriverUser.borrow.user(userTestIdentifier).getAllReservations().
        should.eventually.be.fulfilled.
        then(function(data) {
            expect(data).to.have.property('length', amount);
        }).
