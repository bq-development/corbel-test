    ...
    describe('while using an admin user', function () {
      ...
      it('assets are retrieved sorted upwardly', function (done) {
        corbelDriver.assets.asset().getAll({
          ...
          .should.be.eventually.fulfilled
          .then(function (response) {
            expect(corbelTest.common.resources.checkSortingAsc(response.data, 'name')).to.be.equal(true)
          })
          .should.notify(done)
      })
      ...
      it('assets are retrieved sorted downwardly', function (done) {
        corbelDriver.assets.asset().getAll({
          ...
          .should.be.eventually.fulfilled
          .then(function (response) {
            expect(corbelTest.common.resources.checkSortingDesc(response.data, 'name')).to.be.equal(true)
          })
          .should.notify(done)
      })
    })
