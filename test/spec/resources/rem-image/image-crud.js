    ...
    describe('when performing CRUD operations', function () {
      if (window.chrome) {
        ...
        afterEach(function (done) {
          corbelDriver.resources.resource(FOLDER_NAME, FILENAME).delete({ dataType: 'image/png' }).should.be.eventually.fulfilled.then(function () {
            return corbelDriver.resources.resource(FOLDER_NAME, FILENAME).get({dataType: 'image/png'}).should.be.eventually.rejected
          }).then(function (e) {
        ...
        it('image is correctly uploaded', function (done) {
          corbelDriver.resources.resource(FOLDER_NAME, FILENAME).update(
            ...
            return corbelDriver.resources.resource(FOLDER_NAME, FILENAME).get({dataType: 'image/png'}).should.be.eventually.fulfilled
          }).should.notify(done)
        })
        ...
        it('image cannot be retrieved without dataType param', function (done) {
          corbelDriver.resources.resource(FOLDER_NAME, FILENAME).update(
            ...
            return corbelDriver.resources.resource(FOLDER_NAME, FILENAME).get().should.be.eventually.rejected
          }).then(function (e) {
            expect(e).to.have.property('status', 404)
            expect(e).to.have.deep.property('data.error', 'not_found')
          }).should.notify(done)
        ...
        it('image is correctly delected', function (done) {
          corbelDriver.resources.resource(FOLDER_NAME, FILENAME).update(
            ...
            return corbelDriver.resources.resource(FOLDER_NAME, FILENAME).delete({dataType: 'image/png'}).should.be.eventually.fulfilled
          }).then(function () {
            return corbelDriver.resources.resource(FOLDER_NAME, FILENAME).get({dataType: 'image/png'}).should.be.eventually.rejected
          }).then(function (e) {
            expect(e).to.have.property('status', 404)
