        ...
        describe('When retrieving a cached image', function() {
            ...
            if (window.chrome) {
                it('cache image is removed after updating the original', function(done) {
                    var cacheResourceName = 'cachedImage' + FILENAME + IMAGE_OPS;

                    corbelDriver.resources.resource(CACHE_FOLDER, cacheResourceName ).get(
                        {
                            dataType:'image/png',
                            responseType: 'blob',
                        }
                    ).
                    should.be.eventually.rejected. /* TODO. Should be fullfiled*/
                    then(function() {
	                    return corbelDriver.resources.resource(FOLDER_NAME, FILENAME).update(
	                        base64Image, 
	                        {
                ...
                it('cache image is removed after deleting the original', function(done) {
                    ...
                    should.be.eventually.rejected. /* TODO. Should be fullfiled*/
                    then(function() {
	                    return corbelDriver.resources.resource(FOLDER_NAME, FILENAME).delete().
    	                should.eventually.be.fulfilled;
                    }).
                    then(function() {
                        return corbelDriver.resources.resource(CACHE_FOLDER, cacheResourceName ).get(
