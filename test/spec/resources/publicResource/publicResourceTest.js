describe('In RESOURCES module', function() {
    ...
    describe('if a resource is public', function(done) {
        ...
        beforeEach(function(done) {
            ...
            .then(function(response) {
                ...
                return corbelTestDriver.iam.token()
                    .create();
            })
            .then(function() {
                var testResource = {
    ...
    describe('if a resource public is added', function(done) {
        it.only('that can be added without token.', function(done) {
            var idResource;
            var currentResourcesEndpoint = corbelTest.getConfig().localServices.indexOf('resources') !== -1 ? 
            ...
            .then(function(response) {
                ...
                idResource = locationSplitBar[locationSplitBar.length - 1];
                return corbel.request.send({
                    url: currentResourcesEndpoint + domainId + '/resource/' + 
                        publicResourceTestCollection + '/' +idResource,
                    method: corbel.request.method.GET,
