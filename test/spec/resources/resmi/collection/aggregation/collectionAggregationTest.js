describe('In RESOURCES module', function() {
    ...
    describe.only('In RESMI module, with a collection with some elements,', function() {
        ...
        before(function(done) {
            ...
            corbelTest.resourcesCommon.createdObjectsToQuery(corbelDriver, COLLECTION, amount).
            should.eventually.be.fulfilled.notify(done);
        });

        after(function(done) {
            corbelTest.resourcesCommon.cleanResourcesQuery(corbelDriver).
        ...
        describe('if we get the collection with count aggregation', function() {
            ...
            it('successes returning number elements in the collection', function(done) {
                ...
                then(function(result) {
                    expect(result).to.have.property('count').to.be.equal(amount);
                }).
                should.eventually.be.fulfilled.notify(done);
            });
        });
        ...
        describe('if we get the collection with sum aggregation', function() {
            ...
            it('successes returning the expected sum', function(done) {
                ...
                should.eventually.be.fulfilled.
                then(function(result) {
                    expect(result).to.have.property('sum').to.be.closeTo(sum, 0.000000000001);
                }).
                should.eventually.be.fulfilled.notify(done);
            });
            ...
            it('returns 0 when applied on a non numeric field', function(done) {
                ...
                should.eventually.be.fulfilled.
                then(function(result) {
                    expect(result).to.have.property('sum').to.be.equal(0);
                }).
                should.eventually.be.fulfilled.notify(done);
            });
        });
        ...
        describe('if we get the collection with average aggregation', function() {
            ...
            it('successes returning the expected average', function(done) {
                ...
                should.eventually.be.fulfilled.
                then(function(result) {
                    expect(result).to.have.property('average').to.be.closeTo(sum / amount, 0.000000000001);
                }).
                should.eventually.be.fulfilled.notify(done);
            });
            ...
            it('returns 0 when applied on a non numeric field', function(done) {
                ...
                should.eventually.be.fulfilled.
                then(function(result) {
                    expect(result).to.have.property('average').to.be.equal(0);
                }).
                should.eventually.be.fulfilled.notify(done);
            });
        });
